...
- **Never write anything into `Report Examples/`.** Those are the appraiser's delivered
...
- **Never move, print, or copy a key or a password.** The app's own key stays
  ...
  final four characters so the appraiser can distinguish one key from another. No other
...
- **Never create a markdown file, doc, or note without asking first.** This
  ...
  put it in the chat.
- **Never touch one of the appraiser's real folders to record something the app knows.**
...
**It must run on the appraiser's Windows PC.** On his machine, nothing but Python runs.
...
may not support claims about the appraiser's real folder structures, documents, layouts,
...
Product acceptance and claims about the appraiser's work run on real folders and real
files.
...
anything the appraiser receives.
