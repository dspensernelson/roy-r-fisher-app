...
# The appraiser's own eight folders, in the order his folder template carries them.
...
def jobs_home() -> Optional[Path]:
    ...
    the appraiser chooses this on a screen now, and workspace.py owns the answer.
...
# app proposes can always actually be created on the appraiser's machine.
WINDOWS_FORBIDDEN = '<>:"/\\|?*'


def propose_folder_name(city: str, street: str, engagement: str, year: int) -> str:
    """A folder name in the firm's house style, for the appraiser to accept or edit."""
...
def list_jobs(home: Optional[Path]) -> list[dict]:
    """The jobs the appraiser has marked active, and only those.
...
def create_job(home: Path, name: str) -> dict:
    """Make the job folder and the appraiser's own eight folders inside it.
