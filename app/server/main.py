...
# on the appraiser's PC would have failed. RRF_PHOTO_TEMPLATE still overrides it.
...
class ProposeName(BaseModel):
    ...
    # and nobody has checked what is on the appraiser's Windows machine yet.
...
def create_app() -> FastAPI:
    ...
    @app.get("/api/demo")
    def demo_status():
        """Whether the testing tool is configured on this computer. On the appraiser's
    ...
    def intake(body: Intake):
        ...
        # because at intake the appraiser often does not have it yet.
    ...
    def get_sections(name: str):
        ...
        # Anything the appraiser added that the matrix does not know about is kept and
    ...
    def caption_styles():
        ...
        # so what the appraiser is shown can never drift from what the model is told.
    ...
    def classification_labels():
        ...
        # so what the appraiser can pick can never drift from what the server accepts.
    ...
    def preview_captions(name: str):
        ...
        result so the appraiser is choosing between two real sentences about his own
