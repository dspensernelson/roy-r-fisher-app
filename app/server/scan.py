...
This module only decides how those needs group onto the appraiser's folders, so the
...
# empty. They are the two folders the appraiser fills first, so those counts get
...
def folder_rows(job: Path) -> list:
    """One row per folder the appraiser has, saying what is here and what is missing."""
    result = scan_job(job)
