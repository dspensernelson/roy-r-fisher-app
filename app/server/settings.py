"""Where the appraiser's Anthropic key lives, and how the app reads and changes it.
...
def active_key() -> str:
    ...
    the same answer, and so the appraiser's Windows machine needs no shell.
    """
...
def status() -> dict:
    ...
    four characters so the appraiser can tell one key from another."""
    key = active_key()
