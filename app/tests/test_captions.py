...
def test_neither_style_may_invent_a_compass_direction():
    ...
    the appraiser's retention pond green space. A wrong direction in a delivered report
...
def test_marks_own_pick_survives_a_reload(tmp_path):
    ...
    manifest["caption_style"] = "view"                 # the appraiser overrides the suggestion
...
def test_captions_fill_only_blank_ones(client, monkeypatch):
    ...
    # The appraiser's edit wins: re-run must not touch a non-empty caption
