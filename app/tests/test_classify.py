"""What the appraiser says a file is, remembered outside his folders.
...
def test_a_label_outside_the_list_is_refused(store, tmp_path):
    ...
    with pytest.raises(ValueError):
        classify.set_label(job, "Maps/plat.pdf", "whatever the appraiser typed")
...
def test_a_renamed_file_reads_missing_and_never_present(store, tmp_path):
    ...
    # The record is kept, so the appraiser can still see it and clear it himself.
