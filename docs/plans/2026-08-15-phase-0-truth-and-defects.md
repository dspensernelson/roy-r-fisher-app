...
### Task 7: Show every folder honestly, and let Mark say what a file is
...
**Goal:** Replace the eight guessed folder rows with every folder actually on disk, let Mark open any folder and see the files inside it, and let him confirm what a file is, with the app storing that answer outside his folders and never touching his files.

**Before.** Mark opens a job. He sees eight fixed rows. A folder he made himself is invisible. He cannot see a single filename. A row says "Has deed" because a file somewhere under a path containing "subject information" had "deed" in its name. He cannot correct any of it.
...
3. Size and modification date are recorded when Mark confirms. If the source at that path later differs, the screen says it changed and stops presenting the classification as currently confirmed.
4. The words "Has" and "Still needs" appear nowhere in this slice. The screen may state what files were observed, what Mark classified, and whether a classified source is present, changed, or missing.
5. Classifications live outside Mark's folders, in the app-owned file named in Step 2. `photo-manifest.json` stays a narrow existing exception and that precedent is not widened.
...
**File safety, the hard line.** The implementation may read directory names, filenames, relative paths, sizes, and modification dates. It may not open or read file contents, modify, rename, move, or delete a source file, create anything inside Mark's job folders, follow a symlink outside the job, or infer a classification from a filename. If `test_file_safety.py` fails, stop and report. It is never weakened.
...
Storage is `~/.rrf-classifications.json`, overridable by `RRF_CLASSIFY_FILE` for tests, copying the pattern already proven by `RRF_KEY_FILE` and `RRF_SETTINGS_FILE`. It sits in the home folder because HOW-WE-WORK's Never list says app knowledge never goes into Mark's folders.
...
The job key is the resolved absolute path of the job folder, the same keying `workspace.save_active_jobs` uses. The file key is the path relative to the job root in POSIX form, so one record works on both machines. Nothing is written into, renamed on, or read out of Mark's folders to make this work.
...
`DELETE /api/jobs/{name}/classification`, body `{"file": "..."}`. Removes the app's record only. Touches nothing in Mark's folders. Works on a missing source, which is how he clears a stale record.
...
ones that need Mark's private material skip on machines without it." That
...
source file of Mark's is opened, modified, renamed, moved, or deleted, and
...
- 15 tests skip on this machine. They are the ones that need Mark's private
